...
(* this is extremely bad but it works shrug *)
let run_samples w cam n_samples =
  let build_lst _ =
    init (width * height) (fun x -> get_ray cam (x mod width) (x / width))
  in
  let eval lst = map (fun r -> bounce w r 5) lst in
  let samps = map eval (init n_samples (fun x -> build_lst 0)) in
  fold_left
    (fun a b -> map2 pix_add a b)
    (init (width * height) (fun _ -> pix_broadcast 0.))
    samps
  |> map (fun px -> pix_scale (1. /. float_of_int n_samples) px)

let z = run_samples w cam 10
let opts = { width; height; gamma = 0.5 }
